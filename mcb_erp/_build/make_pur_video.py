"""Render the 12 Purchase / Procurement click-path flows as MP4s.
...
from pur_meta import META, END, SHORT  # noqa: E402

V.FLOW_META.update(META)
V.END_TEXT.update(END)
V.OUTDIR = Path("/Data/odoo19_enterprise/mukticox/video_purchase")
V.BRAND = "MCB PURCHASE"
...
def build(flow):
    ...
    if out:
        no = META[flow][0]
        nice = out.with_name(f"P{no:02d}_MCB_Purchase_{SHORT[flow]}.mp4")
        out.rename(nice)
        print(f"    -> {nice.name}")
    return out
...
if __name__ == "__main__":
    ...
    print("Rendering Purchase flow videos (static frames — no shake):")
    for f in wanted:
        build(f)
